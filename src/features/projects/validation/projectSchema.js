...
export const projectSchema = z.object({
  ...
  shortDescription: z.string().min(10, "Short description is required (min 10 chars)").max(200),
  fullDescription: z.string().min(20, "Full description is required (min 20 chars)"),
  category: z.enum(projectCategories, {
    errorMap: () => ({ message: "Please select a valid category" })
  }),
  technologies: z.string().min(2, "Add at least one technology"), // Keeping it simple as comma-separated string for now, can be array later
